...
window.exportDashboard = function () {
    ...
    exportContainer.innerHTML = `
        <div class="export-layout" style="width: 1920px; min-height: 1080px; padding: 100px; display: flex; flex-direction: column; justify-content: center; background: var(--background-color);">
            <div class="export-header" style="text-align: center; margin-bottom: 80px;">
                <h1 style="font-size: 6rem; font-weight: 400; color: var(--primary-color); letter-spacing: -2px; margin-bottom: 20px;">MEAL MANAGEMENT REPORT</h1>
            ...
            <div style="display: flex; justify-content: space-around; gap: 40px; margin-bottom: 100px; width: 100%;">
                <div style="flex: 1; text-align: center; padding: 50px; border-radius: 30px; border: 3px solid var(--surface-border); background: var(--surface-color);">
                    ...
                    <div style="font-size: 6rem; font-weight: 400; line-height: 1; color: var(--text-main);">${stats.totalMeals}</div>
                </div>
                <div style="flex: 1; text-align: center; padding: 50px; border-radius: 30px; border: 3px solid var(--surface-border); background: var(--surface-color);">
                    <div style="font-size: 2.9rem; color: var(--text-muted); margin-bottom: 15px;">Total Cost</div>
                    <div style="font-size: 6rem; font-weight: 400; line-height: 1; color: var(--text-main);">${stats.totalCost}</div>
                </div>
                <div style="flex: 1; text-align: center; padding: 50px; border-radius: 30px; border: 3px solid var(--surface-border); background: var(--surface-color);">
                    <div style="font-size: 2.9rem; color: var(--text-muted); margin-bottom: 15px;">Meal Rate</div>
                    <div style="font-size: 6rem; font-weight: 400; line-height: 1; color: var(--text-main);">${stats.mealRate.toFixed(2)}</div>
                </div>
            </div>
            ...
            <div style="margin: 0 auto; width: 100%; padding: 80px; border-radius: 50px; border: 3px solid var(--surface-border); background: var(--surface-color);">
                <h3 style="text-align: center; font-size: 4.5rem; margin-bottom: 60px; color: var(--text-main); font-weight: 400;">Balance Overview</h3>
        ...
        return `
                                <tr>
                                    <td style="padding: 25px; text-align: left; font-size: 3.5rem; font-weight: 400;">${m.name}</td>
                                    ...
                                    <td style="padding: 25px; text-align: right; color: ${balance >= 0 ? 'var(--success-color)' : 'var(--danger-color)'}; font-size: 4rem; font-weight: 400;">
                                        ${balance >= 0 ? '+' : ''}${balance.toFixed(0)}
                                    </td>
                                </tr>
                            `;
    ...
    setTimeout(() => {
        html2canvas(exportContainer.querySelector('.export-layout'), {
            ...
            canvas.toBlob(blob => {
                const fileName = `Mess_Report_${month.replace(' ', '_')}.png`;
                shareData(blob, fileName);
            }, 'image/png');
            exportContainer.innerHTML = '';
